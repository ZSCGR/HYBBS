...
class Post extends HYBBS {
	...
	public function __construct() {
		...
		if(!IS_LOGIN){
			if(IS_AJAX && IS_POST){
				...
				header("location: ". WWW.URL('user','login'));
				die;
			}
	...
	public function Post(){
		...
		if($this->conf['gold_post'] != 0 || $this->conf['credits_post'] != 0){
			S("Log")->insert(array(
				...
				'content'=>'发表评论 文章ID['.$thread_data['id'].']',
				'atime'=>NOW_TIME
			));
		}
		//{hook a_post_post_9}
	...
	public function Index(){
        ...
        if(IS_GET){ //显示发表主题模板
			...
			//主题数据
            $Thread = S("Thread");
            $this->tid = $id = $Thread->insert(array(
                'fid'=>$forum,
                'uid'=>$this->_user['id'],
                'title'=>$title,
	...
	public function edit(){
		...
		if(IS_POST){
			...
			if($isthread){
				...
				if(!L("Forum")->is_comp($fid,NOW_GROUP,'thread',$this->_forum[$fid]['json']))
					return $this->json(array('error'=>false,'info'=>'你没有权限在该板块发表帖子'));
				...
				$thread_data_posts = S("Thread")->find('posts',array('id'=>$post_data['tid']));


				$count = intval(($thread_data_posts /  $this->conf['postlist']) + 1)+1;
		        for ($i=0; $i < $count; $i++) {
		            $this->CacheObj->rm("post_list_{$post_data['tid']}_desc_{$i}");
		            $this->CacheObj->rm("post_list_{$post_data['tid']}__{$i}");
		        }
			}
			//{hook a_post_edit_4}
			//修改评论内容
...
//var_dump( is_forumg($this->_forum,$this->_user['id'],$data['fid']));
	...
	public function vote(){
		...
		$type = X("post.type"); //类型
		if(!in_array($type,array('thread1','thread2','post1','post2')))
	...
	public function del(){
		...
		//if(NOW_UID != $p_data['uid']){
            M("Chat")->sys_send(
                $p_data['uid'],
                '你的评论被删除 所在主题<a href="'.WWW.URL('thread','',EXP.$p_data['tid']).'" target="_blank">['.M('Thread')->get_title($p_data['tid']).']</a> 操作者:'.NOW_USER
            );
        //}
