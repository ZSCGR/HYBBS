...
<body>
			...
			<div style="position: relative;height: 100%;">
				<section class="search-page">
					...
					<div class="search-radio">
						...
						<input type="radio">
						<label class="a"><a href="{#WWW}{php echo URL('forum','',EXP.$v['id']);}">{$v.name}</a></label>
						{/foreach}
					...
					<div style="margin-bottom:5px">
						...
						<a class="user-box" href="{php echo WWW.URL('my',$v['user']);}" target="_blank">
					...
					<div>
						<h4>
							<a href="{php HYBBS_URL('thread','',EXP.$v['id']);}" title="新窗口打开" target="_blank">{$v.title}</a>
						</h4>
						...
						<a title="新窗口打开" href="{php HYBBS_URL('thread','',EXP.$v['id']);}" class="content-bottom" target="_blank">{php HYBBS_URL('thread','',EXP.$v['id']);}</a>
					...
					<a onclick="return more('{$key}',{$type},this);" class="more" href="{#WWW}search?key={$key}&type={$type}&pageid={$pageid}">更多内容...</a>
	...
	function more(key,type,obj){
		...
		window.history.pushState("","",'/search?key='+key+'&type='+type+'&pageid='+(window.pageid+1));
		$.ajax({
